...
const storeProfileInDB = async (profile, args) => {
  await Profile.sync()
  ...
  //     //     armory: `https://worldofwarcraft.com/${args.origin}-${args.origin}/character/${args.realm}/${args.name}`,
  //     //     warcraftlog: `https://www.warcraftlogs.com/character/${args.origin}/${args.realm}/${args.name}`,
  //     //     raiderio: `https://raider.io/characters/${args.origin}/${args.realm}/${args.name}`
  //     //   })
  //     // }
  //   })
}
...
const getRaidsAvancement = (datas) => {
  ...
  const isBfA_AOTC = datas.achievements.achievementsCompleted.indexOf(13322) > -1
  const isCoS_AOTC = datas.achievements.achievementsCompleted.indexOf(13418) > -1
  const isEP_AOTC = datas.achievements.achievementsCompleted.indexOf(13784) > -1
  ...
  getRaidProgression.map((raid, index) => {
    if (raid.name === 'Bataille de Dazar’alor') {
      buildRaidsFields.push(buildRaidFieldValue(raid.name, raid.bosses, raid.totalBosses, isBfA_AOTC))
    }

    if (raid.name === 'Creuset des Tempêtes') {
      buildRaidsFields.push(buildRaidFieldValue(raid.name, raid.bosses, raid.totalBosses, isCoS_AOTC))
    }

    if (raid.name === 'Palais Éternel') {
      buildRaidsFields.push(buildRaidFieldValue(raid.name, raid.bosses, raid.totalBosses, isEP_AOTC))
    }
  })
...
const initEmbedProfile = async (datas, args) => {
  console.log('>>> profile :', datas.class)
  // console.log('>>> talents :', datas.talents)
  const classIdList = [
    { id: 1, name: 'Warrior' },
    { id: 2, name: 'Paladin' },
    { id: 3, name: 'Hunter' },
  ...
  const embed = {
    color: 0x0099ff,
    author: {
      name: `${datas.name} ${getClassName.name}/${filterSpec.spec.name} from ${datas.realm} - ${
...
export default class WarcraftProfileCommand extends Command {
  ...
  async run (msg, args) {
    ...
    initWarCraft.init().then(async warcraftApi => {
      const characterField = await this._getCharacterProfile(warcraftApi, msg, args)
      const mythicKeystoneField = await this._getKeystoneProfile(warcraftApi, msg, args)
      const links = await generateLinks(args)

      characterField.catch(error => {
        console.log(error)
        return waitMsg.then(sentMessage => {
          return sentMessage.edit(`${msg.author} Sorry nobody find !`)
        })
      })

      characterField.fields.push(links)
      characterField.fields.unshift(mythicKeystoneField)

      return waitMsg.then(sentMessage => {
        sentMessage.edit(`${msg.author} This is what we have found !`)
        return msg.embed(characterField)
      })
    })
  }

...
const buildRaidFieldValue = (name, bossKills, total, aotc) => {
  ...
  Heroic Kills : ${bossKills.heroicKills}/${total}
...
const getRaidsAvancement = (datas) => {
  const getLastRaids = datas.progression.raids.slice(-4)
  ...
  const isNWCAHeadCurve = datas.achievements.achievementsCompleted.indexOf(14068) > -1
  ...
  getRaidProgression.map((raid, index) => {
    ...
    if (raid.name === 'Ny’alotha, la cité en éveil') {
      buildRaidsFields.push(buildRaidFieldValue(raid.name, raid.bosses, raid.totalBosses, isNWCAHeadCurve))
    }
  })

...
const generateTable = (title, heading, content) => {
  ...
  table.setHeading(heading).addRow(content)
  ...
  return new Promise((resolve, reject) => {
    const embedProfile = {
      ...
      description:
        ...
        generateTable(
          'Primary Stats',
          ['Agility', 'Force', 'Stamina', 'Inteligence'],
          [
            `${datas.stats.agi}`,
            `${datas.stats.str}`,
            `${datas.stats.sta}`,
...
const getProfile = (warcraftApi, message, args) => {
  warcraftApi.wow
    ...
    .then(async response => {
      ...
      if (args.subCommand !== 'search') {
        await storeProfileInDB(response.data, args)
      }
      return message.embed(profile)
    })
    .catch(err => {
      console.error(err)
    })
...
export default class WarcraftProfileCommand extends Command {
  ...
  async run (msg, args) {
    ...
    initWarCraft.init().then(async warcraftApi => {
      await getProfile(warcraftApi, msg, args)
